...
export default class DatGUI {
  constructor(main) {
    ...
    this.createTree = main.createTree;
    this.model = null;
    this.meshHelper = null;
    this.sphere = main.sphere;
    this.tree = main.tree;
  }
  ...
  load(main, mesh) {
    ...
    /* Plane Count */
    const Leaves = this.gui.addFolder("Number of leaves");
    const leavesCount = Leaves.add(
      Config.numberofleaves,
      "count",
      100,
      50000
    ).name("count");
    leavesCount.onChange((value) => {
      while (this.tree.children.length > 0) {
        this.tree.remove(this.tree.children[0]);
      }
